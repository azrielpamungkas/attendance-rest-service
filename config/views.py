...
class SubmitAttendance(APIView):
    ...
    def get(self, request):
        try:
            ...
            location = geolocator.reverse(lat+", "+lng).address
        except:
            location = "Antah Berantah"
        if current_lecture(request.user.id):
        ...
        return Response({'status_code': 16,
                         'address': location,
    ...
    def post(self, request):
        if current_lecture(request.user.id):
            ...
            submit_data['name'] = "{} {}".format(
                request.user.first_name, request.user.last_name)
            try:
                submit_data['timetable'] = current_lecture(request.user.id).id
            except:
                return Response('Unauthorized')

            radius = 0.06
            center_point = [
                {'lat': -6.990244896200767,
                 'lng': 110.42050151945978}
            ]
            ...
            # Check Token
            if submit_data['token'] == current_lecture(request.user.id).token:
                # Check Coordinate
                if dis <= radius:
                    del submit_data['lat']
                    del submit_data['lng']
...
class StudentDashboard(APIView):
    ...
    def get(self, request, format=None):
        ...
        try:
            current_lecture = current_lecture(request.user.id)
